...
function mobf_add_mob(mob)
	...
	if mobf_contains(mobf_registred_mob,mob.modname.. ":"..mob.name) then
		minetest.log(LOGLEVEL_ERROR,"MOBF: " .. mob.modname.. ":"..mob.name .. " already known or blacklisted, not registering mob with same name!")
		return
	end 
	...
	minetest.register_entity(":".. mob.modname .. ":"..mob.name,
		...
		--	actions to be done by mob on its own
			on_step = function(self, dtime)
				...
				--visual change hook
				graphics.update_orientation(self,now,dtime)
